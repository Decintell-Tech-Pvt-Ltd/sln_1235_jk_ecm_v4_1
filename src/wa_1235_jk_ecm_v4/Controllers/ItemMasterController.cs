...
namespace wa_1235_jk_ecm_v4.Controllers
{
    public class ItemMasterController : Controller
    {
        ...
        public async Task<ActionResult> Dispatchdetails(string Mode, int ReqValue)
        {
            ...
            string jsonData1 = JsonSerializer.Serialize(json1);
            string apiEndPoint2 = "ItemMaster/CT_GetSKUByRequest";
            ItemMasters.SKUlist = JsonSerializer.Deserialize<SKUlist[]>(await _iGenericMethods.PostDataEcm(apiEndPoint2, jsonData1));
            if (ItemMasters.SKUlist != null && ItemMasters.SKUlist.Length > 0)
            {
                ViewBag.SKUCode = ItemMasters.SKUlist[0].SKUCode;
                ViewBag.SKUDesc = ItemMasters.SKUlist[0].SKUDesc;
            }
            return View(ItemMasters);
        }

        public async Task<IActionResult> CTViewLabelLayout(int ReqValue)
        {
            ViewBag.JWTToken = JwtToken;
            ViewBag.ApiUrl = appSettings?.API_blob_1231;
            ViewBag.BaseBlobPath = appSettings?.BaseBlobPath;
